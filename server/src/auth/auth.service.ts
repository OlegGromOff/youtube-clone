...
export class AuthService {
	...
	async getNewTokens(refreshToken: string) {
		try {
			...
			// Если всё ок, ищем юзера
			const user = await this.userService.byId(result.id)
			return this.buildResponseObject(user)
			
		} catch (error) {
			// Если токен протух (jwt expired) или подпись не совпадает (invalid signature),
			// аккуратно отдаем 401 ошибку. Фронтенд поймет это и перекинет юзера на страницу логина.
